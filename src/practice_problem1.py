...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        nocta = min(space, len(additional_contents))
        sta = ''
        for k in range(nocta):
            sta = sta + additional_contents[k]
        self.contents = self.contents + sta
        stufftr = ''
        for k in range(nocta, len(additional_contents)):
            stufftr = stufftr + additional_contents[k]
        return stufftr
    ...
    def double(self):
        ...
        #    ** TWO **   LINES OF CODE.
        ################################################################
        ty = self.append_string(self.contents)
        return ty
    ...
    def shrink(self, new_volume):
        ...
        contents = self.contents[0: new_volume]
        didntfit = self.contents[new_volume:]
        self.contents = contents
        return didntfit
